...
inline Method getMethod(std::string &method){
	if (method == "GET")
		return GET;
	else if (method == "POST")
		return POST;
...
inline std::string getMethodName(const Method &method) {
	if (method == GET)
		return "GET";
	else if (method == POST)
		return "POST";
...
inline bool isValidCode(int code)	  	{ return (code >= 100 && code < 600); } //!< \returns \c true if the given \p code is a valid status code.
